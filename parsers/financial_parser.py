...
class FinancialParser:
    ...
    def parse_financial_data(self, text: str, document_type: str = None) -> DadosFinanceiros:
        ...
        logger.info(
            event="parsing_start",
            message="Financial data parsing started",
        ...
        if SPECIALIZED_PARSERS_AVAILABLE and self.specialized_parsers:
            ...
            if self.cache:
                ...
                if bank_detection and self.metrics:
                    ...
                    logger.debug(
                        event="cache_hit",
                        message="Bank detection cache hit",
                    ...
                    if self.ml_classifier.should_assist(rule_confidence):
                        ...
                        if ml_prediction:
                            ...
                            if ml_confidence > rule_confidence:
                                logger.info(
                                    event="ml_override",
                                    message="ML classifier override",
                                    ml_bank=ml_bank,
            ...
            if bank_detection:
                ...
                if bank_key in self.specialized_parsers:
                    ...
                    logger.info(
                        event="parser_selection",
                        message="Using specialized parser",
                        bank=bank_key,
                        parser=parser_name,
                        confidence=round(confidence, 3)
                    )
                    ...
                    if hasattr(parser, 'can_parse') and parser.can_parse(text):
                        try:
                            ...
                            logger.info(
                                ...
                                message="Specialized parser completed",
                                bank=bank_key,
                                parser=parser_name,
                            ...
                            logger.warning(
                                ...
                                message="Specialized parser failed, falling back to generic",
                                bank=bank_key,
                                parser=parser_name,
                                error=str(e)
                            )
                    ...
                    logger.info(
                        ...
                        message="No specialized parser available, using generic",
                        bank=bank_key
                    )
                ...
                logger.info(
                    ...
                    message="Bank not detected, using generic parser"
                )
